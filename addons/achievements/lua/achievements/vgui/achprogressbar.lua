...
if system.IsOSX() then
	surface.CreateFont("achv_progressbar", {font = "Helvetica", size = 11, antialias = false, weight = 500})
else
	surface.CreateFont("achv_progressbar", {font = "Tahoma", size = 11, antialias = true, weight = 500})
end

function PANEL:Init()
	self.Label = vgui.Create("DLabel", self)
	self.Label:SetFont("achv_progressbar")
...
function PANEL:UpdateText()
	if !self.m_iMax or !self.m_iMin or !self.m_iValue then return end
	local fDelta = 0
	...
	if (self.m_iMin == 0) then
		self.Label:SetText(string.Comma(self.m_iValue).." / "..string.Comma(self.m_iMax))
	end
end
