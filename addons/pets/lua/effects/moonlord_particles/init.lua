		...
		if !self.Entity:IsDormant() and !self.Entity:GetNoDraw() then
			...
			if LocalPlayer():GetPos():DistToSqr(pos) <= dist then
				...
				if !self.NextEffect or (self.NextEffect < CurTime()) then
					...
					particle = self.Emitter:Add("particle/warp1_warp", pos + Vector(math.Rand(-7.5, 7.5), math.Rand(-7.5, 7.5), math.Rand(-7.5, 7.5)))
				...
				if particle then
					...
					particle:SetVelocity(Vector(math.Rand(-15, 15), math.Rand(-15, 15), math.Rand(-15, 15)))
				end
			end
		end
		return true
