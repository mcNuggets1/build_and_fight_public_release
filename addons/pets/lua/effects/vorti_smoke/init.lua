		...
		if !self.Entity:IsDormant() and !self.Entity:GetNoDraw() then
			...
			if LocalPlayer():GetPos():DistToSqr(pos) <= dist then
				...
				if !self.NextEffect or (self.NextEffect < CurTime()) then
					...
					particle = self.Emitter:Add("particle/particle_smokegrenade1", pos + Vector(math.Rand(-6, 5), math.Rand(-5, 5), math.Rand(-5, 5)))
					self.NextEffect = CurTime() + delay
				end
				if particle then
					particle:SetVelocity(Vector(math.Rand(0, 1), math.Rand(1, 1), math.Rand(0, 1)))
					particle:SetLifeTime(0)
					particle:SetDieTime(1)
					particle:SetStartAlpha(math.Rand(0, 100))
					particle:SetEndAlpha(math.Rand(100, 200))
					particle:SetStartSize(3)
					particle:SetEndSize(1)
					particle:SetAngles(Angle(0, 0, 0))
					particle:SetAngleVelocity(Angle(1, 1, 1))
					particle:SetRoll(math.Rand(0, 360))
					local val = math.random(180, 200)
