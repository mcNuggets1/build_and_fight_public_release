...
TOOL.Information = {
	...
	{name = "right", stage = 1, icon = "gui/rmb.png"},
}
...
if (SERVER) then
	if (!ConVarExists("sbox_maxportalpairs")) then
		CreateConVar("sbox_maxportalpairs", 1, {FCVAR_ARCHIVE, FCVAR_REPLICATED}, "Maximum number of portal pairs which can be created by users.")
	end
end
...
function TOOL:LeftClick(trace)
	...
	if (self:GetStage() == 0) then
		...
		if !IsValid(portal1) or !IsValid(portal2) then return end
...
if (CLIENT) then
	...
	language.Add("tool.witchergate.left", "Wähle den Ort für das erste Portal aus")
	language.Add("tool.witchergate.right", "Platzierung abbrechen")
	language.Add("tool.witchergate.left_next", "Wähle den Ort für das zweite Portal aus")
	language.Add("tool.witchergate.desc", "Kreiere zwei verbundene Portale für leichte Traversierung")
	language.Add("tool.witchergate.color", "Portalfarbe")

	language.Add("Cleaned_portalpairs", "Cleaned up all Witcher Gates")
	language.Add("Cleanup_portalpairs", "Witcher Gates")
end