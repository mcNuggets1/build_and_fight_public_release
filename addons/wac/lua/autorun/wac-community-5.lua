...
timer.Simple(5,function()
	if not found and not WACFrame then
		if CLIENT then
			...
			WACFrame:SetTitle("WAC ist nicht installiert")
...
if CLIENT then
	net.Receive("WAC-ModelDetailWarning", function()
		if GetConVarNumber("r_rootlod")>0 then
			chat.AddText(Color(255,62,62), "WARNUNG: ", Color(255,255,255), "Das Vehikel: \""..net.ReadString().."\" benötigt hohe Modelleinstellungen! (Ändere diese im Optionsmenü)")
			chat.PlaySound()
		end
	end)
end
