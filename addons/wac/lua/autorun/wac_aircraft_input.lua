...
wac.hook("wacAirAddInputs", "wac_aircraft_baseinputs", function()
	...
	wac.aircraft.addControls("Flugkontrolleinstellungen", {
	...
	wac.aircraft.addControls("Generell", {
		Exit = {true, KEY_E},
		FreeView = {true, KEY_SPACE},
		Camera = {true, MOUSE_RIGHT},
	})

	wac.aircraft.addControls("Bewaffnung", {
...
if SERVER then
	...
	wac.hook("VehicleMove", "wac_aircraft_mouseinput", function(p)
		local seat = p:GetVehicle()
			...
			if IsValid(heli) and p.wac.mouseInput and p:GetInfo("wac_cl_air_mouse") == "1" then
				 ...
				 if (p.LocalEyeAngles) then
						 ...
						 vLocal = heli:WorldToLocal(e:GetPos() + p:GetAimVector())
				 end
	...
	wac.hook("VehicleMove", "wac_aircraft_joyinput", function(p) 
			...
			for name, k in pairs(category.list) do
				if !k[3] then
					if GetConVar("wac_cl_air_key_" .. name):GetInt() == key then
						...
						vehicle:receiveInput(name, pressed and 1 or 0, ply:GetNWInt("wac_passenger_id"))
					end
				else
					...
					if command and target then
						--[[if GetConVar("wac_cl_air_smoothkeyboard"):GetBool() then
							...
							vehicle:receiveInput(command, target, ply:GetNWInt("wac_passenger_id"))
						--end
					end
				end
			end
	...
	hook.Add("InitPostEntity", "wac_cl_aircraft_smoothkeyboard", function()
		ply = LocalPlayer()
		--[[wac.hook("Tick", "wac_cl_aircraft_smoothkeyboard", function() -- Fixes the banning issue for now.
			if GetConVar("wac_cl_air_smoothkeyboard"):GetBool() then
				local v = ply:GetVehicle()
				if !IsValid(v) then return end
				local vehicle = v:GetNWEntity("wac_aircraft")
				...
				for command, info in pairs(vehicle.inputCache) do
					if info.current != info.target then
						...
						vehicle:receiveInput(command, info.current, ply:GetNWInt("wac_passenger_id"))
					end
				end
			end
		end)]]
	...
	net.Receive("wac_joyinput_usermessage", function(length)
		local vehicle = ply:GetVehicle():GetNWEntity("wac_aircraft")
		if !IsValid(vehicle) then return end
		local s = net.ReadString()
		local b = net.ReadBit()
		vehicle:receiveInput(s, b, ply:GetNWInt("wac_passenger_id"))
	end)
