...
function ENT:Think()
	...
	if !self:GetNWBool("locked") then
		...
		tb.engineRpm = tb.engineRpm - (tb.engineRpm-self:GetNWFloat("engineRpm"))*frt*10

		local engineVal = math.Clamp(tb.engineRpm*100+tb.engineRpm*tb.smoothUp*3+doppler, 0, 200)
		local val = math.Clamp(tb.rotorRpm*100 + doppler + self:GetVelocity():Length()/50, 0, 200)

		tb.sounds.Engine:ChangePitch(engineVal/1.1 + val/10, 0.1)
		tb.sounds.Engine:ChangeVolume(volume*math.Clamp(engineVal*engineVal/4000, 0, inVehicle and 1 or 20), 0.1)
		tb.sounds.Blades:ChangePitch(math.Clamp(val, 10, 150), 0.1)
		tb.sounds.Blades:ChangeVolume(volume*math.Clamp(val*val/5000, 0, inVehicle and 0.4 or 20), 0.1)
		if tb.sounds.Start then
			tb.sounds.Start:ChangeVolume(volume*math.Clamp(100 - tb.engineRpm*110, 0, 100)/100, 0.1)
			tb.sounds.Start:ChangePitch(100 - tb.engineRpm*20, 0.1)
		end
...
function ENT:attachmentThink()
	...
	if !tb.weaponAttachments or !tb.Camera then return end
	local camAng = self:getCameraAngles()
	if !camAng then return end
	local tr = util.QuickTrace(self:LocalToWorld(tb.Camera.pos)+camAng:Forward()*20, camAng:Forward()*999999999, self)
	for _, t in pairs(tb.weaponAttachments) do
...
function ENT:viewCalcThirdPerson(k, p, view)
	...
	local tr = util.TraceLine({
			start = origin,
			endpos = origin + ang:Forward()*-self.thirdPerson.distance,
			filter = {self.Entity, self:GetNWEntity("wac_air_rotor_rear"), self:GetNWEntity("wac_air_rotor_main")},
			mask = 33636363
		})
...
function ENT:viewCalcCamera(k, p, view)
	local first_seat = self.Camera.seat == 1
	if first_seat then
		self.camera:SetAngles(self:GetAngles())
	end
	...
	if self.viewTarget then
		self.viewTarget.angles = first_seat and self:GetAngles() or (p:GetAimVector():Angle() - self:GetAngles())
	end
	self.viewPos = nil
...
function ENT:viewCalc(k, p, pos, ang, fov)
	...
	local valid = IsValid(p:GetVehicle())
	if valid and p:GetVehicle():GetNWEntity("wac_aircraft") != self or !valid then
...
function ENT:MovePlayerView(k,p,md)
	...
	if (k==1 and p:GetInfo("wac_cl_air_mouse")=="1" and (p.wac and !p.wac.viewFree)) then
...
function ENT:Draw()
	...
	if self.engineRpm > 0.2 and self.SmokePos and IsValid(self.Emitter) then
