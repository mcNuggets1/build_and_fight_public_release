...
if SERVER then
	...
	function ENT:Think()
		...
		if self.InFlight then
			...
			if self.InFlight then
				...
				self.flightvector = self:GetUp() * 30 + Vector(0, 0, -0.02)
	...
	function ENT:Explode(tr)
		...
		edata:SetMagnitude(16)
		util.Effect("m9k_gdcw_cinematicboom", edata, true, true)
		util.BlastDamage(self, self.Owner, self:GetPos(), 450, 150)
		util.ScreenShake(self:GetPos(), 10, 5, 1, 2500)
...
if CLIENT then
	...
	function ENT:Think()
		if self:GetNWBool("smoke") and (self.NextParticle or 0) <= CurTime() then
			...
			local Smoke = self.Emitter:Add("particle/smokesprites_000"..math.random(1, 9), self:GetPos() + (self:GetUp() * -40))
			if Smoke then
				Smoke:SetVelocity((self:GetUp() * -2000) + (VectorRand() * 100))
				Smoke:SetDieTime(math.Rand(1.8, 2.5))
