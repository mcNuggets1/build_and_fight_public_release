...
if SERVER then
	...
	function ENT:Think()
		...
		if self.InFlight then
			...
			if self.InFlight then
				...
				self.flightvector = self:GetForward() * 30 + Vector(0, 0, -0.025)
	...
	function ENT:SuicideExplode()
		...
		util.BlastDamage(self, self.Owner, pos, 600/2, 150/2)
...
if CLIENT then
	...
	function ENT:Think()
		if self:GetNWBool("smoke") and (self.NextParticle or 0) <= CurTime() then
			...
			local Smoke = self.Emitter:Add("particle/smokesprites_000"..math.random(1, 9), self:GetPos() + (self:GetForward() * -50))
			if Smoke then
				Smoke:SetVelocity((self:GetForward() * -2000))
				Smoke:SetDieTime(math.Rand(1.8, 2.4))
				Smoke:SetStartAlpha(math.Rand(25, 35))
				Smoke:SetEndAlpha(0)
				Smoke:SetStartSize(math.Rand(40, 50))
