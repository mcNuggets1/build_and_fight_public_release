...
function ENT:Initialize()
	self:SetModel("models/weapons/tfa_nmrih/w_me_hatchet.mdl")
...
function ENT:PhysicsCollide(data, phys)
	...
	if !self.Disabled and data.Speed > 200 then
		...
		if Ent:IsWorld() and self:WaterLevel() < 3 then
			...
			timer.Simple(0, function()
				if !IsValid(self) then return end
				self:SetPos(hitpos - hitnormal * 8)
				self:SetAngles(hitnormal:Angle() + Angle(40, 0, 0))
				if IsValid(phys) then
					phys:EnableMotion(false)
				end
			end)
...
function ENT:Use(activator)
	if activator:IsPlayer() and !activator:HasWeapon("m9k_hatchet") then
		self:Remove()
		activator:Give("m9k_hatchet")
		activator:SelectWeapon("m9k_hatchet")
	end
end
