...
function ENT:Initialize()
	self:SetModel("models/khrcw2/doipack/w_kabar.mdl")
...
function ENT:PhysicsCollide(data, phys)
	...
	if !self.Disabled and data.Speed > 100 then
		...
		if Ent:IsWorld() and self:WaterLevel() < 3 then
			...
			timer.Simple(0, function()
				if !IsValid(self) then return end
				self:SetPos(hitpos - hitnormal * 2.5)
				self:SetAngles(hitnormal:Angle() + Angle(160, 0, 0))
				if IsValid(phys) then
					phys:EnableMotion(false)
				end
			end)
...
function ENT:Use(activator)
	if activator:IsPlayer() and !activator:HasWeapon("m9k_knife") then
		self:Remove()
		activator:Give("m9k_knife")
		activator:SelectWeapon("m9k_knife")
	end
end
