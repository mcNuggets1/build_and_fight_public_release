...
sounds["sometimes i dream about cheese"] = sounds["cheese"]
...
sounds["freeman"] = {"vo/npc/male01/freeman.wav", "vo/npc/male01/docfreeman01.wav", "vo/npc/male01/docfreeman02.wav", "vo/npc/female01/docfreeman01.wav", "vo/npc/female01/docfreeman02.wav", "vo/npc/female01/freeman.wav", "vo/npc/vortigaunt/freeman.wav"}
...
sounds["cough"] = {"ambient/voices/cough1.wav", "ambient/voices/cough2.wav", "ambient/voices/cough3.wav", "ambient/voices/cough4.wav"}
sounds["ugh"] = {"ambient/voices/citizen_beaten1.wav", "ambient/voices/citizen_beaten2.wav", "ambient/voices/citizen_beaten3.wav", "ambient/voices/citizen_beaten4.wav", "ambient/voices/citizen_beaten5.wav"}
sounds["ahhh"] = {"ambient/voices/f_scream1.wav"}
sounds["ahh"] = {"ambient/voices/m_scream1.wav"}
sounds["acquired"] = {"HL1/fvox/acquired.wav"}
sounds["time, doctor freeman"] = {"vo/Citadel/gman_exit01.wav"}
sounds["take some ammo"] = {"vo/npc/male01/ammo04.wav"}

if SERVER then
	local cnt
	hook.Add("PlayerSay", "B&F_ChatSounds", function(ply, text)
		if !ply:Alive() then return end
		if (ply.NextSpeechSound or 0) > CurTime() then return end
		local txt = string.lower(text)
		local tb = sounds[txt]
		if tb then
			local canChatSound = hook.Run("canChatSound", ply, txt)
			if canChatSound == false then return end
			ply.NextSpeechSound = CurTime() + 3
			ply:EmitSound(table.Random(tb), 80, 100)
			hook.Run("onChatSound", ply, txt)
		end
	end, HOOK_MONITOR_LOW or 2)
end

if CLIENT then
	concommand.Add("chatsounds", function(ply)
