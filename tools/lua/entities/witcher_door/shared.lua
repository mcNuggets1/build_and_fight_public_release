...
elseif (CLIENT) then
	...
	function ENT:Think()
		...
		if !tb or !tb.dt then return end

		if (LocalPlayer():EyePos():DistToSqr(self:GetPos()) <= 1000000) then
			local light = DynamicLight(self:EntIndex())

			if (light) then
				local vecCol = tb.dt.RealColor
		...
		if (!IsValid(tb.hole)) then
			tb.hole = ClientsideModel("models/hunter/plates/plate1x2.mdl", RENDERGROUP_BOTH)
			tb.hole:SetPos(self:GetPos() - self:GetUp() * (1 + offset))
			tb.hole:SetAngles(self:GetAngles())
			tb.hole:SetParent(self)
			tb.hole:SetNoDraw(true)
			tb.hole:EnableMatrix("RenderMultiply", matrix)
		end

		if (!IsValid(tb.top)) then
			tb.top = ClientsideModel("models/hunter/plates/plate075x1.mdl", RENDERGROUP_BOTH)
			tb.top:SetMaterial("portal/border")
			tb.top:SetPos(self:GetPos() + self:GetRight() * 44.5 - self:GetUp() * (12.5 + offset))
			tb.top:SetParent(self)
			tb.top:SetLocalAngles(Angle(-75, -90, 0))
			tb.top:SetNoDraw(true)
			tb.top:EnableMatrix("RenderMultiply", matrix)
		end

		if (!IsValid(tb.bottom)) then
			tb.bottom = ClientsideModel("models/hunter/plates/plate075x1.mdl", RENDERGROUP_BOTH)
			tb.bottom:SetMaterial("portal/border")
			tb.bottom:SetPos(self:GetPos() - self:GetRight() * 44.5 - self:GetUp() * (12.5 + offset))
			tb.bottom:SetParent(self)
			tb.bottom:SetLocalAngles(Angle(-75, 90, 0))
			tb.bottom:SetNoDraw(true)
			tb.bottom:EnableMatrix("RenderMultiply", matrix)
		end

		if (!IsValid(tb.left)) then
			tb.left = ClientsideModel("models/hunter/plates/plate075x2.mdl", RENDERGROUP_BOTH)
			tb.left:SetMaterial("portal/border")
			tb.left:SetPos(self:GetPos() + self:GetForward() * 20.8 - self:GetUp() * (12.5 + offset))
			tb.left:SetParent(self)
			tb.left:SetLocalAngles(Angle(-75, 0, 0))
			tb.left:SetNoDraw(true)
			tb.left:EnableMatrix("RenderMultiply", matrix)
		end

		if (!IsValid(tb.right)) then
			tb.right = ClientsideModel("models/hunter/plates/plate075x2.mdl", RENDERGROUP_BOTH)
			tb.right:SetMaterial("portal/border")
			tb.right:SetPos(self:GetPos() - self:GetForward() * 20.8 - self:GetUp() * (12.5 + offset))
			tb.right:SetParent(self)
			tb.right:SetLocalAngles(Angle(-105, 0, 0))
			tb.right:SetNoDraw(true)
			tb.right:EnableMatrix("RenderMultiply", matrix)
		end

		if (!IsValid(tb.back)) then
			tb.back = ClientsideModel("models/hunter/plates/plate3x3.mdl", RENDERGROUP_BOTH)
			tb.back:SetMaterial("vgui/black")
			tb.back:SetPos(self:GetPos() - self:GetUp() * 42)
			tb.back:SetParent(self)
			tb.back:SetLocalAngles(angle_zero)
			tb.back:SetNoDraw(true)
		end
	end
	...
	function ENT:DrawTranslucent()
		...
		local up = self:GetUp()
		if (InFront(EyePos(), pos - up * 1.8, up))then return end

		local tb = self:GetTable()
		if !tb or !tb.dt then return end

		local color = tb.dt.RealColor
		local elapsed = CurTime() - tb.dt.AnimStart
		local frac = math.Clamp(elapsed / 0.5, 0, 1)

		if (frac <= 1) then
			tb.h, tb.s, tb.l = ColorToHSL(Color((color.x / 2) * 255, (color.y / 2) * 255, (color.z / 2) * 255))
			tb.l = Lerp(frac, tb.l or 1, 0)
			tb.col = HSLToColor(tb.h, tb.s, tb.l)
		end

		tb.lerpr = Lerp(frac, tb.lerpr or 255, tb.col.r)
		tb.lerpg = Lerp(frac, tb.lerpg or 255, tb.col.g)
		tb.lerpb = Lerp(frac, tb.lerpb or 255, tb.col.b)
		...
		render.ClearBuffersObeyStencil(tb.lerpr, tb.lerpg, tb.lerpb, 0, true)
		...
		local viewNorm = (pos - EyePos()):GetNormalized()
		local dot = viewNorm:Dot(up * -1)

		if (dot >= 0) then
			render.SetColorModulation(1, 1, 1)
			local visible = util.PixelVisible(pos + up * 3, 20, tb.PixVis)

			if (!visible) then return end

			local alpha = math.Clamp((EyePos():Distance(pos) / 10) * dot * visible, 0, 30)
			local newColor = Color((color.x / 2) * 255, (color.y / 2) * 255, (color.z / 2) * 255, alpha)

			render.SetMaterial(mat)
			render.DrawSprite(pos + up * 2, 600, 600, newColor, visible * dot)
		end
	end
end