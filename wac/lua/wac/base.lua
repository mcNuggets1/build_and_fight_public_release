...
local rip = function(dist, a, b, delta)
    ...
    dist = math.abs(dist)
    b = b / 4
    return math.min(math.max((math.pow((math.pow(dist, 1.0 / 6) / b + delta) * b, 6) - dist), a * delta), dist) * (negative and -1 or 1)
end
...
wac = wac or {
	...
	sprinting = function(p)
		if p and IsValid(p) then
			local b = ((p:KeyDown(IN_SPEED) and (p:GetVelocity():LengthSqr()+100)>10000))
			return b
		end
		return false
	end
