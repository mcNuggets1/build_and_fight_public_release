ENT.PrintName = "Milkor Mk1-Grenade"
ENT.Type = "anim"
...
if SERVER then
	...
	function ENT:Think()
		...
		if self.InFlight then
			...
			if self.InFlight then
				...
				self.flightvector = self:GetUp() * 30 + Vector(0, 0, -0.04)
	...
	function ENT:SuicideExplode()
		...
		edata:SetScale(1.4)
		edata:SetRadius(50)
		edata:SetMagnitude(14)
		util.Effect("m9k_gdcw_cinematicboom", edata, true, true)
		util.BlastDamage(self, self.Owner, self:GetPos(), 300/2, 200/2)
		util.ScreenShake(self:GetPos(), 10/2, 5/2, 1/2, 2500/2)
		util.Decal("Scorch", self:GetPos(), self:GetPos() - Vector(0, 0, 5), self)
		self:Remove()
	end
	...
	function ENT:Explode(tr)
		...
		edata:SetMagnitude(14)
		util.Effect("m9k_gdcw_cinematicboom", edata, true, true)
		util.BlastDamage(self, self.Owner, tr.HitPos, 300, 200)
...
if CLIENT then
	...
	function ENT:Think()
		if self:GetNWBool("smoke") and (self.NextParticle or 0) <= CurTime() then
			...
			local Smoke = self.Emitter:Add("particle/smokesprites_000"..math.random(1, 9), self:GetPos() + (self:GetUp() * -55))
			if Smoke then
				Smoke:SetVelocity((self:GetUp() * -2000) + (VectorRand() * 100))
				Smoke:SetDieTime(math.Rand(1.5, 2.3))
				Smoke:SetStartAlpha(math.Rand(15, 25))
				Smoke:SetEndAlpha(0)
				Smoke:SetStartSize(math.Rand(30, 45))
				Smoke:SetEndSize(math.Rand(95, 110))
