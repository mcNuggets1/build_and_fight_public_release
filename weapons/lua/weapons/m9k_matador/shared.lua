...
SWEP.WorldModel = Model("models/weapons/w_gdcw_matador_rl.mdl")
...
SWEP.Primary.Ammo = "matador_round"
SWEP.Primary.Round = "m9k_gdcwa_matador_90mm"
SWEP.CanAim = false
SWEP.RunSightsPos = Vector(2.4946, -1.5644, -1)
...
function SWEP:PrimaryAttack()
	...
	if self:CanPrimaryAttack() and !self:GetRunning() and !self:GetPredictedRunning() and !self:GetReloading() then
		...
		self.Owner:ViewPunch(Angle(-3, util.SharedRandom(self:GetClass(), -1, 1, 0), 0))
		if SERVER then
			self:FireGrenade()
